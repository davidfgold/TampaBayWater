...
n_formulations = 2; f_type = ['SCH Pipeline', 'SCH Pipeline + SWTP Exp.']; f_num = [141,143]
...
#    ax4.set_xticklabels([])
    ax3.set_xticklabels([str(x) for x in range(2020,2041,5)])
    ax4.set_xticklabels([str(x) for x in range(2020,2041,5)])
...
#    ax6.set_xticklabels([str(x) for x in range(2020,2041,5)])
    ...
    # add legend
#    ax2.legend(loc = 'upper right', title = 'Uniform Rate Policy', labels = sim_type[sim])
...
#    ax5.set_ylabel('Scenario 3 (143):\nPipeline + Desal + 20MGD SWTP Exp.')
    ...
    plt.savefig(data_path + '/Simulation_Covenant_Comparisons_NewRuns_141_143_' + str(sim) + '.png', bbox_inches= 'tight', dpi = 800)
